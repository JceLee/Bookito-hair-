...
export default function MessengerListView() {
    const [currentUser, setCurrentUser] = useState(useSelector((state) => state.signIn.currentUser));
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            firebaseDate.ref('rooms/').on('value', resp => {
                ...
                setRoom(rooms.filter(room => room.customerID === currentUser.uid || room.designerID === currentUser.uid));
            });
        };
    ...
    return (
        <div>
            <Divider />
            {room.map((item, idx) => (
                <MessengerListCard fname={(item.designerID === currentUser.uid) ? item.customerID : item.designerID} photoURL={null} enterChatRoom={enterChatRoom} roomID={item.roomID}/>
            ))}
        </div>
    )
}
