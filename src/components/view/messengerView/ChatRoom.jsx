...
import { Form, Input, Container, Button, InputGroupAddon, InputGroup} from 'reactstrap';
...
function ChatRoom(props) {
    ...
    const [newChat, setNewChat] = useState({ roomID: '', nickname: '', message: '', date: '', type: '' });
    const history = useHistory();

    useEffect(() => {
        const fetchData = async () => {
            setNickname(currentUser.uid);
            firebaseDate.ref('chats/').orderByChild('roomID').equalTo(roomID).on('value', resp => {
                setChats([]);
            ...
            <ScrollToBottom className="chatContent">
                {chats.map((item, idx) => (
                    <div className="chatMessage">
                        <div className={`${item.nickname === currentUser.fname ? "rightBubble":"leftBubble"}`}>
                ...
                <Form className="messageForm" onSubmit={submitMessage}>
                    <InputGroup className="inputWrapper">
                        ...
                        <InputGroupAddon className="submitWrapper" addonType="append">
                            <Button className="sendMessage" variant="primary" type="submit">Send</Button>
                        </InputGroupAddon>
                    </InputGroup>
                    </Form>
            </footer>
