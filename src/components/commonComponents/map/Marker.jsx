...
export default function Marker(props) {
    ...
    return designer ? (
        ...
        //     content={
                ...
                //     </Link>
                //     <div style = {{margin: 10}}>
                //         <Link to={`/designer_profile?uid=${designer.uid}`}><h4 style={{color: "#332C1E", marginBottom: 0}}>{designer.name}</h4></Link>
            ...
            >
                    <div className={"popoverContent"}>
                        ...
                        <div style = {{ marginLeft: 10 }}>
                            <Link to={`/designer_profile?uid=${designer.uid}`}><h4 style={{color: "#332C1E", marginBottom: 4}}>{designer.name}</h4></Link>

                            {/* <h4 style={{color: "#332C1E", marginBottom: 0}}>{designer.name}</h4> */}
                            <StarRead style= {{marginBottom: 4}} rateScore={4.32} rateCount={32}/>
                            <p style={{color: "#332C1E", marginTop: 2}}>{`${getDistanceFromLatLonInKm(lat, lng, userLocation && userLocation.lat, userLocation && userLocation.lng)}km from you`}</p>
                            <div style={{width: 170}}>
                                {designer.services && designer.services.map((service, index) => (
                                    <Tag shape="rounded" style={{marginBottom: 5, borderRadius: 20}} color={tagColors[index%tagColors.length]}>{service}</Tag>
                                ))}
